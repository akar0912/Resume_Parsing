...
def Degree(text):
    ...
    return list(set(f))
#------------------------------------------------------------------------------#
path = "C:\\Users\\AlishaRavikaKar\\Desktop\\res\\resumes\\"
data = []
...
for filename in os.listdir(path):
    if filename.endswith(('.pdf')):
        ...
        df["Role"]= df['Exprerience'] .apply(lambda x: "Associate" if (x>=3 and x<5) else "Analyst" if (x>2 and x<3) else "partner")   